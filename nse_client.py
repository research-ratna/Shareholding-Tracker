...
def iter_latest_shareholding_filings(nse: NSE, symbols: list[str]) -> Iterator[dict]:
    ...
    (the .shareholding() call already returns latest-quarter-first). Used
    for the daily scan across the full symbol universe, where we mainly
    want to notice a new quarter as soon as it's filed, not re-walk every
    symbol's whole history every day.
    ...
    """
    for record in iter_all_shareholding_filings(nse, symbols, latest_only=True):
        yield record


def iter_all_shareholding_filings(nse: NSE, symbols: list[str], latest_only: bool = False) -> Iterator[dict]:
    """
    Like iter_latest_shareholding_filings(), but by default yields EVERY
    filing record .shareholding() returns per symbol (however many past
    quarters NSE's API exposes - in practice a handful, not a company's
    full listed history), oldest concerns aside, in latest-first order.
    Used for historical backfill: on the company watchlist (small, so this
    is cheap) and for a one-off investor-side --backfill run (expensive
    across the full universe - see run.py).
    """
    for symbol in symbols:
        try:
            records = nse.shareholding(symbol)
        except Exception as e:  # noqa: BLE001 - deliberately broad, this can loop 2000x
        ...
        wanted = records[:1] if latest_only else records
